...
//=======クロスマダンテの判定処理が可能になった場合グローバル変数をアップデートして保持する=======
//④グローバル変数アップデート関数
...
int OrderFuncrion(string Currency, int EntryOrderFlag)
{
  ...
  else if (OrderFlag == 2)
  {
    ...
    price = Bid;
    slippage = 30;
    stoploss = Bid+200*Point;
    takeprofit = Bid-200*Point;
    comment = "";
    magic = MAGICMA;
    expiration = 0;
    arrow_color = clrRed;
...
int CrossMadantePerfectOrder(string Currency)
{
  ...
  if (get_MA_5 > get_MA_14 && get_MA_14 > get_MA_21 && get_MA_21 > get_MA_60 && get_MA_60 > get_MA_240 && get_MA_240 > get_MA_1440){
    if(get_MA_5 > pre_get_MA_5 && get_MA_14 > pre_get_MA_14 && get_MA_21 > pre_get_MA_21 && get_MA_60 > pre_get_MA_60 && get_MA_240 > pre_get_MA_240 && get_MA_1440 > pre_get_MA_1440){
      //ローソク足の雲の内外存在判定処理
...
//買いポジションの場合、価格が上昇したら、その上昇した価格の20ポイント下にロスカットラインを引き上げる設定を行う
//======⑨トレーリングストップ設定処理======
